...
void Game::play()
{
	srand(time(0));
	// Init Cast
	contestant_controller.buildCast(location_controller);
	contestant_controller.equipCast(weapon_controller);
	int cast_size = contestant_controller.getContestants()->size();
	int turn_counter = 1;
	while (contestant_controller.checkForWinner() == false)
	{
		cout << " - TAG " << turn_counter << " - " << endl;
		for (int i = 0; i < cast_size; i++)
		{
			string output = turn(contestant_controller.getContestant(i));
			if (output != "")
			{
				cout << output << endl;
			}
		}
		contestant_controller.refreshAP();
		turn_counter++;
		cout << endl;
	}
	cout << "And the winner is: " + contestant_controller.accessWinner() + "!";
}
	...
	if (t_actor->checkVital())
	{
		if (!(contestant_controller.checkIfLastSurvivor(*t_actor)))
		{
			...
			if (contestant_controller.canStillAct(*t_actor))
			{
				...
				if (contestant_controller.canStillAct(*t_actor))
				{
					output = contestant_controller.stageFight(t_actor);
				}
			}
		}
	}
...
string Game::locationEventCornucopia(Contestant* t_actor)
{
	...
	// Event found nothing 5%
	if (5 < random && random <= 10)
	{
		output.append(t_actor->getName() + " searches the cornucopia for supplies and finds nothing.");
		t_actor->setActionPoints(0);
	}

	// Event found something 15%
	if (10 < random && random <= 25)
	{
	...
	if (5 < random && random <= 15)
	{
		...
		if (!stach.isNull())
		{
			if (stach.getDamage() > t_actor->getWeapon().getDamage())
			{
				if (!(t_actor->getWeapon().isNull()))
				{
					output.append(t_actor->getName() + " found a hidden weapon stash! Inside was a(n) " + stach.getName() + ". " + contestant_controller.getPronoun(*t_actor, "pronoun_cap") + " decides to equip it, throwing away " + contestant_controller.getPronoun(*t_actor, "possesive_pronoun") + " " + t_actor->getWeapon().getName() + ".");
					t_actor->setActionPoints(0);
					t_actor->setWeapon(stach);
				}
				else
				{
					output.append(t_actor->getName() + " found a hidden weapon stash! Inside was a(n) " + stach.getName() + ". " + contestant_controller.getPronoun(*t_actor, "pronoun_cap") + " decides to equip it.");
					t_actor->setActionPoints(0);
				}
			}
			else
			{
				output.append(t_actor->getName() + " found a hidden weapon stash! Inside was a(n) " + stach.getName() + ". " + contestant_controller.getPronoun(*t_actor, "pronoun_cap") + " decides to keep " + contestant_controller.getPronoun(*t_actor, "possesive_pronoun") + " " + t_actor->getWeapon().getName() + ".");
				t_actor->setActionPoints(0);
			}
		}
	}
